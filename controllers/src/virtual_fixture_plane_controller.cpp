...
{
    class VirtualFixturePlaneController : public controller_interface::ControllerInterface
    {
    public:
        VirtualFixturePlaneController() {}
        ~VirtualFixturePlaneController()
        {
            if (data_logger_)
                data_logger_->save(FileUtils::getHomeDirectory() + "/experiment_logs/vf_plane/", "vf_plane");
        }
        ...
        CallbackReturn on_configure(const rclcpp_lifecycle::State & /*previous_state*/) override
        {
            ...
            node_->get_parameter_or<std::vector<double>>("Ku", Ku_vec_, {1500.0});
            node_->get_parameter_or<std::vector<double>>("Bu", Bu_vec_, {50.0});
        ...
        void update(const rclcpp::Time &t, const rclcpp::Duration &period) override
        {
            ...
            Ju_ = Eigen::Vector3d(0, 0, 1).transpose() * Jh_.bottomRows(3);
            dJu_ = Eigen::Vector3d(0, 0, 1).transpose() * dJh_.bottomRows(3);
...
PLUGINLIB_EXPORT_CLASS(controllers::VirtualFixturePlaneController, controller_interface::ControllerInterface)