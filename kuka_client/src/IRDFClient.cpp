...
IRDFClient::IRDFClient() : _isFirstUpdate(true),
...
{
    updateRobotStatus();
    // _robotCommand.setJointPosition(_robotState.getC);
}
//******************************************************************************
void IRDFClient::waitForCommand()
{
    cout << "wait for command " << _robotState.getClientCommandMode() << endl;
    //  updateRobotStatus();
...
void IRDFClient::updateRobotStatus()
{
    ...
    memcpy(&_iiwaStatus.ipoJointPosition, _robotState.getIpoJointPosition(), NUMBER_OF_JOINTS * sizeof(double));
...
//******************************************************************************
void IRDFClient::getRobotIpoJointPosition(std::vector<double> &jp)
{
    for (auto i = 0ul; i < jp.size(); i++)
        jp[i] = _iiwaStatus.ipoJointPosition[i];
}
//******************************************************************************
